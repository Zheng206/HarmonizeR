...
server <- function(input, output, session) {
  ...
  extract_stan_indices <- function(x) {
    ...
    inside <- sub("\\]$", "", inside)
    if (identical(inside, as.character(x)) || !nzchar(inside)) return(integer(0))
    suppressWarnings(as.integer(strsplit(inside, ",")[[1]]))
  }
  
  get_first_index <- function(vars) {
    idx <- lapply(vars, extract_stan_indices)
    sapply(idx, function(z) if (length(z) >= 1 && !is.na(z[1])) z[1] else NA_integer_)
  }
  
  get_second_index <- function(vars) {
    idx <- lapply(vars, extract_stan_indices)
    sapply(idx, function(z) if (length(z) >= 2 && !is.na(z[2])) z[2] else NA_integer_)
  }
  ...
  filter_mcmc_summary <- function(smry, family, batch = NULL, modality = NULL) {
    smry <- standardize_mcmc_summary(smry)
    if (is.null(smry) || is.na(family) || !nzchar(family)) return(data.frame())
  ...
  # Extract harmonized data from the RDS exported by the downloaded MCMC script.
  # This lets uploaded MCMC results reuse the existing Post-Harmonization pipeline
  # without changing the Post-Harmonization plots.
  ...
  vectorize_shrink_object <- function(x, batch = NULL) {
    ...
    if (is.matrix(x)) {
      if (!is.null(batch) && !identical(as.character(batch), "All")) {
        ...
        if (!is.na(b_int) && b_int >= 1 && b_int <= nrow(x)) return(as.numeric(x[b_int, ]))
      }
      return(as.numeric(x))
    }
    ...
    if (is.list(x)) {
      if (!is.null(batch) && !identical(as.character(batch), "All")) {
        nm <- names(x)
        if (!is.null(nm) && as.character(batch) %in% nm) return(as.numeric(unlist(x[[as.character(batch)]])))
        b_int <- suppressWarnings(as.integer(gsub("[^0-9]", "", as.character(batch))))
        if (!is.na(b_int) && b_int >= 1 && b_int <= length(x)) return(as.numeric(unlist(x[[b_int]])))
      }
  ...
  # These guards are especially important for uploaded longitudinal data and
  # for modality CSVs re-uploaded from the MCMC project folder. In those files,
